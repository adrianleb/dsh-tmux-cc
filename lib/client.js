...
`
    ...
    function fitPane(rec, pane) {
      ...
      rec.fitRaf = requestAnimationFrame(() => {
        ...
        // Mobile reading floor: never fit below MOBILE_MIN_FONT (or the
        // preferred size when the user chose an even smaller one). The grid
        // then overflows and syncPaneOverflow makes the overflow pannable.
        const minimum = mobile ? Math.min(MOBILE_MIN_FONT, preferred) : 5
        const key = `${cols}x${rows}:${hw}x${hh}:${preferred}:${mobile ? 'm' : 'd'}:${window.devicePixelRatio || 1}`
     ...
     * scrollback at all. One-finger drags map to the pane, unless the pane
     * program enabled mouse reporting (then drags belong to the program):
     *
     * - Vertical drags pan the clipped grid rows first, then continue into
     *   xterm scrollback, in the natural direction (content follows the
     *   finger: drag down reveals older lines) and with release momentum.
     * - Horizontal drags pan across a grid wider than the pane box, which is
     *   the normal case now that mobile fonts stop at a readable floor.
     *
     * Gestures hold the paint guard so a snapshot repaint cannot jiggle the
     * pane mid-drag, and every native fallback is fenced: the terminal host
     * uses touch-action:none, so nothing here ever scrolls the page.
     */
    function bindTouchScroll(rec) {
      ...
      let axis = ''    // 'y' pans rows + scrollback, 'x' pans a wide grid
      let tracking = false
      let guarded = false
      let acc = 0
      let flingRaf = 0
      ...
      /** Vertical budget: consume clipped grid rows first, then scrollback. */
      const scrollY = (dy) => {
        const term = rec.term
        ...
        if (rest !== 0) {
          ...
          if (lines !== 0) {
            try { term.scrollLines(lines) } catch { return used }
            acc -= lines * rowH
          }
          used = true
        }
