...
@media (max-width:767px){
  ...
  [data-tmux-cc-shell][data-mobile="1"] [data-tmux-cc-bar]{
    display:grid;grid-template-columns:minmax(0,1fr);grid-template-rows:44px 40px 48px;
    height:auto;min-height:136px;gap:2px;padding:calc(4px + env(safe-area-inset-top)) 8px 4px;
    border-bottom:1px solid var(--dsw-alias-border-l2);
  }
...
`
    ...
    function paint(host, store) {
      ...
      if (!shell) {
        ...
        const zoom = mk(t('zoom'), '<path d="M8 3H5a2 2 0 0 0-2 2v3"/><path d="M16 3h3a2 2 0 0 1 2 2v3"/><path d="M8 21H5a2 2 0 0 1-2-2v-3"/><path d="M16 21h3a2 2 0 0 0 2-2v-3"/>', () => store.send({ type: 'zoom' }))
        zoom.setAttribute('data-tmux-cc-zoom', '')
        actions.append(
