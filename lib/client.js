window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function createStore() {
      ...
      const api = {
        panes,
        get: () => ({ prefs, snapshot, error, notice }),
...
@media (max-width:767px){
  ...
  [data-tmux-cc-shell][data-mobile="1"] .xterm-helper-textarea{
    font-size:16px !important;left:0 !important;top:0 !important;
  }
...
`
    ...
    function fitPane(rec, pane) {
      ...
      rec.fitRaf = requestAnimationFrame(() => {
        ...
        if (mobile) {
          minimum = floor
          // With the untouched default floor, prefer showing the FULL grid
          // width whenever that stays readable (>= 9px): seeing the whole
          // mirror beats panning for typical phone-sized tmux windows. Any
          // explicit stepper choice pins the floor exactly instead.
          if (floor === MOBILE_MIN_FONT && gw > 0) {
            const widthFit = Math.floor((cur * hw / gw + 1e-6) * 4) / 4
            if (widthFit >= 9 && widthFit < minimum) minimum = widthFit
          }
        }
    ...
    function paint(host, store) {
      ...
      if (!shell) {
        ...
        const toggleKbd = () => {
          ...
          // Diagnostic breadcrumb: if the on-screen keyboard has not
          // actually appeared shortly after a mobile summon, say why-ish in
          // the drawer so the failure mode is reportable at a glance.
          if (!isNarrowViewport() || !window.visualViewport) return
          const vvBefore = window.visualViewport.height
          window.setTimeout(() => {
            if (!isNarrowViewport() || !store.get().prefs.open) return
            const vv = window.visualViewport
            const shrank = vv && vv.height < vvBefore - 60
            if (!dockTerminalFocused()) {
              const active = document.activeElement
              store.notify(`keyboard: focus lost → ${active ? active.tagName.toLowerCase() : 'none'}`)
            } else if (!shrank) {
              store.notify(`keyboard: focused, not shown (vv ${Math.round(vv ? vv.height : 0)}/${window.innerHeight})`)
            }
          }, 1400)
        }
