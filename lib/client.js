  ...
  factory: (require) => {
    ...
    function loadPrefs() {
      try {
        ...
        const source = document && document.version === PREFS_VERSION && document.prefs
          ? document.prefs
          : document
...
/* Hardware keyboards need no toggle; shown on mobile below. */
[data-tmux-cc-kbd]{display:none}
...
@media (max-width:767px){
  ...
  [data-tmux-cc-shell][data-mobile="1"] [data-tmux-cc-kbd]{display:inline-flex}
...
`
    ...
    function mobileViewportInsets() {
      ...
      const rawRight = window.innerWidth - (vv.width + (vv.offsetLeft || 0))
      const rawBottom = window.innerHeight - (vv.height + (vv.offsetTop || 0))
      return {
        top,
        right: rawRight > 1 ? Math.max(0, Math.round(rawRight)) : 0,
        bottom: rawBottom > 1 ? Math.max(0, Math.round(rawBottom)) : 0,
        left,
      }
    }
    ...
    function placeShell(shell, prefs) {
      ...
      if (isNarrowViewport()) {
        ...
        // Sub-2px inset jitter (URL-bar settling, scroll rounding) must not
        // move the shell: every reposition re-fits fonts and flashes panes.
        const prevTop = Number(shell.dataset.insetTop)
        const prevRight = Number(shell.dataset.insetRight)
        const prevBottom = Number(shell.dataset.insetBottom)
        const prevLeft = Number(shell.dataset.insetLeft)
        const same = shell.dataset.mobile === '1'
          && Number.isFinite(prevTop) && Math.abs(prevTop - inset.top) < 2
          && Number.isFinite(prevRight) && Math.abs(prevRight - inset.right) < 2
          && Number.isFinite(prevBottom) && Math.abs(prevBottom - inset.bottom) < 2
          && Number.isFinite(prevLeft) && Math.abs(prevLeft - inset.left) < 2
        shell.dataset.mobile = '1'
        if (same) return
        shell.dataset.insetTop = String(inset.top)
        shell.dataset.insetRight = String(inset.right)
        shell.dataset.insetBottom = String(inset.bottom)
        shell.dataset.insetLeft = String(inset.left)
      ...
      delete shell.dataset.insetTop
      delete shell.dataset.insetRight
      delete shell.dataset.insetBottom
      delete shell.dataset.insetLeft
    ...
    function fittedFontSize(current, screenWidth, screenHeight, hostWidth, hostHeight, preferred = 18) {
      const scale = Math.min(hostWidth / screenWidth, hostHeight / screenHeight)
      const target = current * scale
      return Math.max(5, Math.min(preferred, Math.floor((target + 1e-6) * 4) / 4))
    }
    ...
    function fitPane(rec, pane) {
      ...
      rec.fitRaf = requestAnimationFrame(() => {
        ...
        const key = `${cols}x${rows}:${hw}x${hh}:${preferred}:${window.devicePixelRatio || 1}`
        if (rec.fitKey === key) return
        rec.fitKey = key
        const cur = term.options.fontSize || preferred
        const next = fittedFontSize(cur, gw, gh, hw, hh, preferred)
        const neutral = (term.options.letterSpacing || 0) === 0 && (term.options.lineHeight || 1) === 1
        if (Math.abs(next - cur) < 0.25 && neutral) return
        try {
    ...
    function mountPane(body, pane, store) {
      ...
      if (!rec) {
        ...
        wrap.addEventListener('pointerdown', (ev) => {
          ...
          // The toolbar keyboard button opens input explicitly.
        ...
        body.append(wrap)
        rec = {
          wrap, label, termHost, term: null, seeded: false, fitRaf: null, fitKey: '',
     ...
     * the program). Horizontal-led gestures are left alone. The mapping is
     * natural (content follows the finger: drag down reveals older lines),
     * releases with momentum, and holds the paint guard so a snapshot repaint
     * cannot jiggle the pane mid-gesture.
     */
    function bindTouchScroll(rec) {
      ...
      let lastY = 0
      let lastT = 0
      let velocity = 0 // px/ms, positive = towards newer output
      let tracking = false
      let guarded = false
      let decided = false
      let acc = 0
      ...
      const scrollBy = (dy) => {
        ...
        if (lines !== 0) {
          try { term.scrollLines(lines) } catch { return false }
          acc -= lines * rowH
        }
        return true
      }
      ...
      host.addEventListener('touchmove', (ev) => {
        ...
        if (mode && mode !== 'none') { tracking = false; release(); return }
        ...
        if (!decided) {
          ...
          if (dy < 8 && dx < 8) return
          decided = true
          if (dx > dy) { tracking = false; release(); return }
        }
        gestureGuard.touch()
        // Content follows the finger: drag down → older scrollback above.
        const step = lastY - y
      ...
      host.addEventListener('touchend', () => {
        tracking = false
        // Momentum: decay the release velocity so long scrollback is reachable.
        ...
        if (Math.abs(velocity) > 0.15 && rec.term) {
          let v = velocity
          ...
          const tick = (now) => {
            ...
            if (Math.abs(v) < 0.05 || !rec.term || !scrollBy(v * dt)) {
              release()
              return
            }
            gestureGuard.touch()
      ...
      host.addEventListener('touchcancel', () => { tracking = false; stopFling(); release() }, { passive: true })
    }
    ...
    function paint(host, store) {
      ...
      if (!shell) {
        ...
        const kbd = mk(t('kbd'), '<rect x="2" y="4" width="20" height="16" rx="2"/><path d="M6 8h.01"/><path d="M10 8h.01"/><path d="M14 8h.01"/><path d="M18 8h.01"/><path d="M8 12h.01"/><path d="M12 12h.01"/><path d="M16 12h.01"/><path d="M7 16h10"/>', () => {
      ...
      for (const key of ['split-h', 'split-v', 'zoom', 'kill', 'kbd']) {
    ...
    function bindKeys(host, store) {
      ...
      const onKey = (event) => {
        ...
        if (event.key.toLowerCase() === 'x') { swallow(event); store.requestKill('active'); return }
    ...
    function mountDock(store) {
      ...
      // (the on-screen keyboard follows that focus on mobile).
      const updateKbd = () => {
        const btn = host.querySelector('[data-tmux-cc-kbd]')
        if (!btn) return
        const focused = document.activeElement
        const on = !!(focused && host.contains(focused) && focused.closest && focused.closest('.xterm'))
      ...
      const onViewportScroll = () => {
        // Scrolling the visual viewport must only move the mobile shell. A full
        // paint here re-fit every pane on every iOS scroll frame.
