...
`
    ...
    function setMobileLock(on) {
      ...
      // keyboard instead of overlaying it (interactive-widget, honored by
      // Chromium; harmlessly ignored elsewhere). A layout viewport that
      // tracks the keyboard leaves no leftover range for the browser-level
      // pan that steals drag gestures and shows the page underneath.
      try {
    ...
    function bindTouchScroll(rec) {
      ...
      host.addEventListener('touchcancel', (ev) => {
        ev.stopPropagation()
        tracking = false
        stopFling()
        release()
