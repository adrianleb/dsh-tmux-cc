...
一个用于 DeepSeek Harness Web 的持久化 **tmux 控制模式工作台**。它可以创建会话或通过 `tmux -C` 连接已有会话，管理已连接客户端，用 xterm.js 渲染每个窗格，并且在切换聊天时始终保持可见。
...
- **会话管理** —— 创建会话（可指定起始目录）、重命名或连接已有会话、切换窗口，以及按需启动可选的命名会话方案。
- **已连接客户端管理** —— 查看配置的 tmux 服务器上的所有原生客户端，按会话筛选并安全地断开所选客户端，不终止进程。
...
- tmux 与 DSH 安装在同一台主机上（已在 tmux 3.4 和 3.7b 验证）
- Linux 或 macOS
...
5. 用鼠标或手指拖动窗格分隔条，可横向或纵向调整大小；桌面端也可拖动工作台外边缘调整大小。使用标签切换 tmux 窗口。

为了避免意外终止整个会话，本插件拒绝关闭会话中的最后一个窗格。

### 会话与客户端

点击工具栏中的 **会话与客户端**（网格加号图标）。即使工作台未连接或没有任何会话，也可以使用。

- **创建会话：** 输入唯一名称，可选填 DSH 主机上现有目录的绝对路径。新会话启动主机的默认 shell；取消勾选 **创建后连接** 可保持其未连接状态。名称允许空格和 Unicode，最多 200 个字符，不支持点、冒号、分号或控制字符。不能使用指向其他会话的已配置方案 ID 作为名称，以免连接目标发生歧义。
- **会话：** 查看窗口和客户端数量、连接会话或就地重命名。连接操作会切换所有浏览器查看者共享的工作台；重命名不会重启进程。
- **已连接客户端：** 查看配置的 tmux 服务器（不含其他 socket）上所有原生客户端的名称、会话、PID、终端/TTY、尺寸、标志和连接时间。多个浏览器标签页共享一个控制客户端，不会单独列出。
- 按会话筛选、逐个选择或 **全选**，然后点击 **断开所选客户端** 并确认目标。改变筛选条件会清除选择。主机会核对客户端的名称、PID 和创建时间，避免过期选择误伤替换后的客户端。
- **此停靠栏（共享）** 会列出但不能批量断开。请使用工作台原有的 **断开** 按钮断开共享控制客户端。断开不会终止会话或进程；不提供结束会话或服务器的按钮。

管理器在可见且空闲时每五秒刷新一次，也可点击 **刷新** 立即更新。错误会保留在界面上，不清空输入。如果连接中断或请求超时，请刷新后再重试：操作可能已完成，客户端不会自动重放修改请求。点击 **完成** 或按 `Escape` 关闭管理器。
...
- 字体固定为移动端工具栏选定的大小（默认 12px），键盘弹出或收起不会改变字号。超出窗格可视范围的网格可用单指横向或纵向惯性平移；主动平移或回滚后保留阅读位置，否则保持提示符底部对齐。
- 将窗格标题拖到另一个窗格可交换原生 tmux 位置；拖动终端内容只滚动，不移动窗格，也不改变键盘焦点。
- 纵向拖动能滚动任何类型的窗格内容：越过被裁剪的网格行后，手势转换为合成滚轮事件，由 xterm 按窗格状态解释——启用鼠标上报的程序（agent CLI、TUI）收到真实滚轮上报并滚动自己的对话记录，备用屏幕程序收到方向键，普通缓冲区滚动 xterm 回滚。
...
- 拖动窗格边界上的可见手柄，可横向或纵向调整窗格大小。手机和触控设备的分隔条采用 24px 触控宽度；拖动终端内容仍只滚动。仅隐藏全屏工作台的外边缘拖动条和桌面端方向选择器，主要按钮采用 44px 触控区域。
...
窗格关闭确认默认开启。单次操作会弹出明确的确认对话框，取消后窗格保持不变；禁用确认后可单击关闭窗格。工作台的“收起”按钮始终单击生效，不会终止进程。主机仍会拒绝关闭会话中的最后一个窗格。
